...
$idNewAnotherValue = $_POST['idNewAnotherValue'];
$idNewAnotherValue = htmlspecialchars($idNewAnotherValue,ENT_QUOTES);

$typeAnother = $_POST['typeAnother'];
    $typeAnother = htmlspecialchars($typeAnother,ENT_QUOTES);

$describeAnotherGood = $_POST['describeAnotherGood'];
    $describeAnotherGood = htmlspecialchars($describeAnotherGood,ENT_QUOTES);


$quantityNewAnother = $_POST['quantityNewAnother'];
    $quantityNewAnother = htmlspecialchars($quantityNewAnother,ENT_QUOTES);

$colorAnother = $_POST['colorAnother'];
    $colorAnother = htmlspecialchars($colorAnother,ENT_QUOTES);

$firstPriceAnother = $_POST['firstPriceAnother'];
    $firstPriceAnother = htmlspecialchars($firstPriceAnother,ENT_QUOTES);

$lastPriceAnother = $_POST['lastPriceAnother'];
    $lastPriceAnother = htmlspecialchars($lastPriceAnother,ENT_QUOTES);
...
$res = $mysql->query("SELECT `cod` FROM `all_avaible_goods` WHERE `cod` = '$idNewAnotherValue'");
...
if( $count > 0 ) {
 ...
 $mysql->query("INSERT INTO `all_avaible_goods`(`type_good`,`good_content`,`color`,`cod`,`quantity`,`first_price`,`last_price`,`id_photo`)
    VALUES('$typeAnother','$describeAnotherGood','$colorAnother','$idNewAnotherValue','$quantityNewAnother','$firstPriceAnother','$lastPriceAnother','$fileName')");
  $output = "SELECT * FROM `all_avaible_goods` WHERE `cod` = '$idNewAnotherValue'";
